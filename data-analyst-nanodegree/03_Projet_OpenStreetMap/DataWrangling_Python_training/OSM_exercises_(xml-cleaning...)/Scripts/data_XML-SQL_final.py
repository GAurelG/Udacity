...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node':
        ...
        for child in element:
            if child.tag == 'tag':
                dict_tag = sub_tag(child, node_attribs["id"],  problem_chars)
        ...
        for child in element:
            if child.tag == 'tag':
                dict_tag = sub_tag(child, way_attribs["id"], problem_chars)
                tags.append(dict_tag)
            elif child.tag == 'nd':
                position = len(way_nodes)
                node_id = child.attrib['ref']
                way_id = way_attribs['id']
                way_nodes.append({'id' : way_id, 'node_id' : node_id, 'position' : position})
        print way_nodes
        print element.tag
        return {'way': way_attribs, 'way_nodes': way_nodes, 'way_tags': tags}

def sub_tag(child, tag_id,  problem_chars, lower_colon = LOWER_COLON):
    """create the sub tag element"""
